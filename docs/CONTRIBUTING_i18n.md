...
Thanks for helping localize Clash Orbit. This guide reflects the current architecture, where the React frontend and the Tauri backend keep their translation bundles separate. Follow the steps below to keep both sides in sync without stepping on each other.
...
- If you touch backend copy, edit the matching YAML file in `crates/clash-orbit-i18n/locales/<lang>.yml`.
...
Native UI strings (tray menu, notifications, dialogs) use `rust-i18n` with YAML bundles stored in `crates/clash-orbit-i18n/locales/<lang>.yml`. These files are completely independent from the frontend JSON modules.
...
- Rust code resolves the active language through the `clash-orbit-i18n` crate (`crates/clash-orbit-i18n/src/lib.rs`). No additional build step is required after editing YAML files; `tauri dev` and `tauri build` pick them up automatically.
...
3. If the backend should expose the language, create `crates/clash-orbit-i18n/<new-lang>.yml` and translate the keys used in existing YAML files.
