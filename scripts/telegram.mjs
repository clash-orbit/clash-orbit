...
const CHAT_ID_RELEASE = '-1004327123943' // 正式发布频道（Clash Orbit / @clash_orbit）
const CHAT_ID_TEST = '-1004327123943' // 测试通知同上
...
async function sendTelegramNotification() {
  ...
  const downloadUrl =
    ...
    `https://github.com/clash-orbit/clash-orbit/releases/download/v${version}`
  ...
  const content = `<b>🎉 <a href="https://github.com/clash-orbit/clash-orbit/releases/tag/${releaseTag}">Clash Orbit v${version}</a> ${releaseTitle}</b>\n\n${formattedContent}`
    ...
    await axios.post(
      `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,
      {
        ...
        link_preview_options: {
          is_disabled: false,
          url: `https://github.com/clash-orbit/clash-orbit/releases/tag/v${encodedVersion}`,
