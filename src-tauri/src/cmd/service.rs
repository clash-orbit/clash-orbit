    ...
    FallbackFuture: std::future::Future<Output = anyhow::Result<()>>,
{
    ...
    let Some(failure) = error.downcast_ref::<clash_orbit_service_ipc::management::InstallationVerificationError>()
    else {
    ...
    let reasons = failure
        ...
        .filter_map(|core| match &core.availability {
            clash_orbit_service_ipc::CoreAvailability::Rejected { reason } => Some(format!("{}: {reason}", core.name)),
            _ => None,
        })
        .collect::<Vec<_>>();
    if reasons.is_empty() {
