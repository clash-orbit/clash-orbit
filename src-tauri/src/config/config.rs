use super::{IClashTemp, IOrbit, IProfiles, MixedPort};
use crate::{
...
impl Config {
        ...
        CONFIG
            .get_or_init(|| async {
                Self {
                    ...
                    orbit_config: Draft::new(IOrbit::new().await),
    ...
    pub async fn orbit() -> Draft<IOrbit> {
        Self::global().await.orbit_config.clone()
    }
    ...
    pub async fn init_config_before_window() -> Result<()> {
        ...
        clash_orbit_i18n::sync_locale(orbit.language.as_deref());
    ...
    pub(crate) async fn runtime_config_yaml() -> Result<std::string::String> {
        ...
        AsyncHandler::spawn_blocking(move || {
            ...
            Ok(format!("# Generated by Clash Orbit\n\n{}", yaml_str))
        })
        .await
    ...
    pub async fn apply_all_and_save_file() {
        ...
        let save_orbit_task = AsyncHandler::spawn(|| async {
            let orbit = Self::orbit().await;
            orbit.apply();
            logging_error!(Type::Config, orbit.data_arc().save_file().await);
        });
        ...
        let _ = tokio::join!(save_clash_task, save_orbit_task, save_profiles_task);
