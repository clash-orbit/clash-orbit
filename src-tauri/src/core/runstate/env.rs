...
impl RunStateEnv for RealEnv {
    ...
    async fn trusted_install_evidence(&self) -> Result<bool> {
        ...
        #[cfg(unix)]
        if !registered && let Err(error) = clash_orbit_service_ipc::execution::check_sidecar_available().await {
            return Ok(error
                .downcast_ref::<clash_orbit_service_ipc::execution::ResidualServiceError>()
                .is_some());
        }
        Ok(registered)
    }
