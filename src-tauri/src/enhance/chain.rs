...
impl ChainItem {
    ...
    pub fn builtin() -> Vec<(ChainSupport, Self)> {
        // meta 的一些处理
        let meta_guard = Self::to_script("orbit_meta_guard", include_str!("./builtin/meta_guard.js"));

        // meta 1.13.2 alpn string 转 数组
        let hy_alpn = Self::to_script("orbit_hy_alpn", include_str!("./builtin/meta_hy_alpn.js"));

        // meta 的一些处理
        let meta_guard_alpha = Self::to_script("orbit_meta_guard", include_str!("./builtin/meta_guard.js"));

        // meta 1.13.2 alpn string 转 数组
        let hy_alpn_alpha = Self::to_script("orbit_hy_alpn", include_str!("./builtin/meta_hy_alpn.js"));
...
impl ChainSupport {
    pub fn is_support(&self, core: Option<&String>) -> bool {
        match core {
            Some(core) => matches!(
                (self, core.as_str()),
                (Self::ClashMeta, "orbit-mihomo") | (Self::ClashMetaAlpha, "orbit-mihomo-alpha")
            ),
            None => true,
        }
    }
