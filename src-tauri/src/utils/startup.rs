...
pub(crate) fn report_error(error: &anyhow::Error) {
    ...
    eprintln!("[clash-ORBIT] startup failed: {detail}");
    ...
    let message = match log_result {
        Ok(path) => format!(
            "Clash Orbit could not start.\n\n{detail}\n\nDiagnostic log:\n{}",
            path.display()
        ),
        Err(log_error) => {
            eprintln!("[clash-ORBIT] failed to write startup log: {log_error:#}");
            format!(
    ...
    let _ = rfd::MessageDialog::new()
        ...
        .set_title("Clash Orbit startup failed")
