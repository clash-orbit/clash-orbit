...
it.each([
  ...
  async (_state, visible, minimized) => {
    ...
    const [handlers, onSubscribed] =
      vi.mocked(subscribeOrbitEvents).mock.calls[0]
    onSubscribed?.()
    handlers['orbit://notice-message']?.(['core_start::error', ''])
    await new Promise((resolve) => setTimeout(resolve, 0))
...
it('shows the same core failure again after the core has started in between', async () => {
  ...
  const [handlers] = vi.mocked(subscribeOrbitEvents).mock.calls[0]
  handlers['orbit://notice-message']?.(['core_start::error', ''])
  await new Promise((resolve) => setTimeout(resolve, 0))

  // The core started and stopped again before the window handled either event.
  handlers['orbit://run-state-changed']?.({ mode: 'Service' } as RunState)
  handlers['orbit://notice-message']?.(['core_start::error', ''])
  await new Promise((resolve) => setTimeout(resolve, 0))
